...
const UserDetailAdmin = () => {
  ...
  const getEstadoPresupuesto = (presupuesto) => {
    ...
    if (porcentaje < 70)
      return { clase: "text-success bg-success-100", estado: "Controlado" };
    if (porcentaje < 99)
      return { clase: "text-warning bg-warning-100", estado: "Limitado" };
  ...
  if (errorUsuario) {
    return (
      <ErrorScreen
        ...
        mensaje={
          errorUsuarioData?.message || "Ha ocurrido un error inesperado."
        }
        tipoError="error"
      />
    );
  ...
  return (
    <div className="container mx-auto p-4 min-h-[calc(100vh-4rem-2.5rem)]">
      {/* Header */}
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-xl font-bold text-aguazul">Detalle de Usuario</h1>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-3">
          <div className="flex items-center min-w-0 flex-1">
            <div className="h-14 w-14 rounded-full border-2 border-neutral-300 flex items-center justify-center bg-neutral-100 mr-3 flex-shrink-0">
              {usuario.avatar ? (
                <img
                  src={avatarFullUrl}
                  alt={`${usuario.nombre}`}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Badge de estado */}
            {usuario.estado ? (
              <div className="flex items-center text-success bg-success-100 px-2 py-1 rounded-full">
                <CheckCircle size={14} className="mr-1" />
                <span className="text-sm font-medium">Activo</span>
              </div>
            ) : (
              <div className="flex items-center text-error bg-error-100 px-2 py-1 rounded-full">
            ...
            {/* Dropdown Menu */}
            <DropdownMenu
              triggerIcon={<MoreVertical size={18} className="text-neutral-600" />}
              actions={[
                {
                  label: usuario.estado ? "Desactivar usuario" : "Activar usuario",
                  onClick: () => setMostrarConfirmacion(true),
                  className: usuario.estado ? "text-error hover:bg-error-50" : "text-success hover:bg-success-50"
                }
              ]}
            />
          </div>
        ...
        {/* Email y último acceso juntos */}
        <div className="space-y-1 text-neutral-600">
          <div className="flex items-center min-w-0">
            <Mail size={14} className="mr-2 flex-shrink-0" />
            <span className="break-all text-sm">{usuario.email}</span>
          </div>
            ...
            <p className="text-sm text-neutral-600">
              {cargandoCuentas ? (
                <span className="flex items-center">
                  <Loader size={12} className="animate-spin mr-1" /> Cargando
                  cuentas...
                </span>
              ) : (
          ...
          <div className="max-h-80 overflow-y-auto">
            {cargandoCuentas ? (
              <div className="flex justify-center items-center py-6">
                <Loader size={20} className="animate-spin text-aguazul" />
              </div>
            ) : errorCuentas ? (
              ...
              <div className="p-3 text-center text-neutral-600">
                ...
                <Wallet size={28} className="mx-auto mt-2 text-neutral-300" />
              </div>
            ) : (
              <ul className="divide-y divide-neutral-200">
                {cuentas.map((cuenta) => (
                  <li key={cuenta.id} className="p-3 hover:bg-neutral-50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-neutral-600">
                            {getNombreTipo(cuenta.tipo)} • Actualizado:{" "}
                            {formatearFecha(
                              cuenta.last_update || cuenta.updated_at
                            )}
                          </p>
                        </div>
                      </div>
                      ...
                      <p
                        className={`font-semibold text-sm ${
                          cuenta.balance >= 0 ? "text-success" : "text-error"
                        }`}
                      >
                        {mostrarSaldos
          ...
          <div className="p-4 border-b border-neutral-200">
            ...
            <p className="text-sm text-neutral-600">
              {cargandoPresupuestos ? (
                ...
                `${presupuestos.filter((p) => p.estado).length} de ${
                  presupuestos.length
                } categorías activas`
              )}
              {errorPresupuestos && (
          ...
          <div className="max-h-80 overflow-y-auto">
            {cargandoPresupuestos ? (
              ...
              <div className="p-3 text-center text-neutral-600">
                ...
                <PieChart size={28} className="mx-auto mt-2 text-neutral-300" />
              </div>
            ) : (
              <ul className="divide-y divide-neutral-200">
                {presupuestos.map((presupuesto) => {
                  const porcentajeProgreso = Math.min(
                    100,
                    (presupuesto.progreso / presupuesto.limite) * 100
                  );
                  const estadoInfo = getEstadoPresupuesto(presupuesto);
                  const esActivo = presupuesto.estado;
                  const diasRestantes = esActivo
                    ? calcularDiasRestantes(presupuesto.fecha_fin)
                    : 0;

                  return (
                    <li key={presupuesto.id} className="p-3">
                      ...
                      <div className="flex justify-between items-center text-xs">
                        <div>
                          ...
                          <p className="font-medium">
                            {mostrarSaldos
                              ? formatearMoneda(presupuesto.progreso)
                              : "••••••"}
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="font-medium">
                            {mostrarSaldos
                              ? formatearMoneda(presupuesto.limite)
                              : "••••••"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-1 text-xs text-neutral-500">
                        {formatearFecha(presupuesto.fecha_inicio)} -{" "}
                        {formatearFecha(presupuesto.fecha_fin)}
                      </div>
                    </li>
                  );
                })}
      ...
      <ConfirmModal
        ...
        } a ${usuario.nombre} ${usuario.apellidos}?`}
